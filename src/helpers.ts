...
export const initializeTaskFile = () => {
    return new Promise((resolve, reject) => {
        ...
        date.stdout.on('data', (data) => {
            ...
            if(fs.existsSync(filePath)) {
                console.log(
                    chalk.red(`Task file already initialized at ${filePath}`)
                )
            } else {
...
export const addTask = (taskSection, task="New Task") => {
        ...
        .stdout.on('data', 
            (date) => { 
                ...
                let grep = exec(`grep -n -F "[${isDefault ? 'TODO' : taskSection}]" ${taskDir}/${currentFile} | sed 's/\\n//g'`, (err) => {
                    if(err) {
                        console.log(
                            chalk.red('Error adding task to taskfile...')
                        )
                    }
                })
                grep.stdout.on('data', 
